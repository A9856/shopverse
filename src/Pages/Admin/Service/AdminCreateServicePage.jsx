...
export default function AdminCreateServicePage() {
    ...
    function postData(e) {
        ...
        if (error)
            ...
            let item = ServiceStateData.find(x => x.name.toLowerCase() === data.name.toLowerCase())
            if (item) {
                setErrorMessage((old) => {
                    return {
                        ...old,
            ...
            <div className="container-fluid my-3">
                <div className="row">
                    ...
                    <div className="col-md-9">
                        <h5 className='bg-secondary p-2 text-light text-center'>Create Service <Link to="/admin/service" className='float-end'><i className='fa fa-long-arrow-left text-light'></i></Link></h5>
                        <form onSubmit={postData}>
                            <div className="row">
                                <div className="col-6 mb-3">
                                    <label>Name*</label>
                                    <input type="text" name="name" onChange={getInputData} placeholder='Service Name' className={`form-control border-3 ${show && errorMessage.name ? 'border-danger' : 'border-secondary'}`} />
                                    {show && errorMessage.name ? <p className='text-danger'>{errorMessage.name}</p> : null}
                                </div>

                                <div className="col-6 mb-3">
                                    <label>Icon*</label>
                                    <input type="text" name="icon" onChange={getInputData} placeholder='Service Icon Tag' className={`form-control border-3 ${show && errorMessage.icon ? 'border-danger' : 'border-secondary'}`} />
                                ...
                                <div className='col-12 mb-3'>
                                    ...
                                    <textarea name="description" onChange={getInputData} placeholder='Description' className={`form-control border-3 ${show && errorMessage.description ? 'border-danger' : 'border-secondary'}`} ></textarea>
